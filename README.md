...
- **Dual voice persona**: Zeev's voice is Groq Orpheus `daniel`; device mode is spoken by **Sarina** (Zeev's secretary) using Kokoro `af_heart` at natural speed — voice selectable per-request via the Go audio daemon
