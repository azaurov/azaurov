...
### 🤖 [Zeev](https://github.com/azaurov/Zeev) · [landing page](https://sogdiana-gematria.net/zeev/)
> *Python · Groq API · Kokoro/Piper TTS · SQLite · Raspberry Pi Zero 2W*

Personal AI companion running on a Raspberry Pi Zero 2W. Auto-routes between a fast, a smart, and a reasoning-tier Groq model per message; falls back to a local Ollama server when offline. Per-model rate-limit tracking automatically falls back to the fast tier when a bigger model hits a Groq daily or burst limit, and to OpenRouter's free tier if Groq itself is rate-limited — no errors surfaced to the user. Vision (camera/photo questions) runs entirely on OpenRouter's free tier, since Groq no longer serves any vision model.
...
- **Dual voice persona**: Zeev's voice is Groq Orpheus `daniel`; device mode is spoken by **Sarina** (Zeev's partner) using Kokoro `af_heart` at natural speed — voice selectable per-request via the Go audio daemon; multilingual TTS (Hebrew/Spanish/Russian) activated by explicit `/lang` command (terminal/web) or spoken request like "speak Russian" (device mode) rather than auto-detection from character sets; Russian and Spanish route to dedicated remote Piper voices rather than English TTS. If the remote Kokoro backend fails mid-reply, the Go daemon reports it back immediately instead of silently re-speaking the whole thing through a lower-quality local fallback, so Zeev retries through cloud TTS rather than repeating itself
- **House cameras**: beyond the Pi's own camera, Zeev looks through Wyze cameras around the house — some by direct RTSP, others via a phone-relay snapshot path, two of those with pan/tilt and a spotlight for a dark yard. Ask it to check on a named pet and it sweeps the relevant cameras, reporting a plain "didn't see them" rather than guessing when nothing matches
- **Reminders & timers**: Zeev's only write capability, via LLM tool calling — "remind me to call at 4" creates a real reminder that's claimed and announced exactly once, waiting for a free moment rather than talking over a live conversation
- **Semantic history recall**: past conversations are embedded remotely and retrieved by vector similarity (with a keyword-index fallback for offline use), recency-weighted and deduplicated so the same question asked on different days doesn't crowd out other context
